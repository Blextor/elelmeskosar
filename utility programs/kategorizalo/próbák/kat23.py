...
MAX_ROWS_PER_PROP  = 8   # egy tulajdonság-csoport legfeljebb ennyi sor magas
...
class TermekTagger:
    def __init__(self, master, termekek, kategoriak_dict, eredmenyek):
        ...
        self.tulajdonsagok_frame.pack(pady=5, fill=tk.BOTH, padx=4)
    ...
    # --------- Görgethető csoport helper (8 soros konténer) ----------
    def _make_scrollable_group(self, parent, rows_to_show):
        """
        Létrehoz egy (legfeljebb 8 sor magas) görgethető konténert.
        Visszaad: (canvas, inner_frame), ahová a sorokat pakolhatod.
        """
        ...
        def _on_inner_config(event):
            ...
            # a belső szélességre igazítjuk a kanvasz szélességét
            canvas_width = event.width
            canvas.itemconfig(inner_id, width=canvas_width)

        inner.bind("<Configure>", _on_inner_config)

        # egérgörgő támogatás csak fókuszban
    ...
    def frissit_tulajdonsagok(self, mentett_ertekek=None):
        ...
        Mindegyik lista-csoport görgethető, legfeljebb 8 sor magas.
        """
        ...
        for nev, spec in tulajd.items():
            ...
            if isinstance(spec, list) or (isinstance(spec, dict) and 'values' in spec):
                ...
                # görgethető konténer (8 soros limit)

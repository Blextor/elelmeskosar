...
DEFAULT_PAGE_URL = "https://www.roksh.com/penny-hu/kezdooldal"
DEFAULT_API_BASE_URL = "https://shopservice.roksh.com"
DEFAULT_PROVIDER_ROUTE = "penny-hu"
DEFAULT_PROVIDER_CODE = "PENNY-HU"
DEFAULT_PROVIDER_ID = 19
...
def fetch_category_tree(session, args):
    ...
    if not isinstance(data, list):
        raise RuntimeError("A Penny kategoria API nem listat adott vissza.")
    return data
    ...
    while total_pages is None or page <= total_pages:
        ...
        try:
            ...
            if args.verbose or page == 1 or page == total_pages:
                print(
                    f"Penny kategoria {category_index}/{category_count}: "
            ...
            print(f"Penny termeklista hiba: {category_row['path']} / oldal {page} - {error}", flush=True)
...
def save_products(products):
    if not products:
        raise RuntimeError("A Penny letoltes 0 termeket adott vissza, ezert nem irok ures all_data fajlt.")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Penny/Roksh kategoriak es termekek letoltese.")
...
def main():
    ...
    with requests.Session() as session:
        ...
        print(
            f"Penny kategoriak: {len(category_rows)} osszesen, "
    ...
    print(f"Penny kategoriak mentve: {args.category_file}")
    print(f"Penny kategoria snapshot: {categories_output}")
    print(f"Penny nyers termekadatok mentve: {products_output}")
    print(f"Penny egyedi termekek: {len(products)}")
    if failed_output:
        print(f"Penny sikertelen lekeresek mentve: {failed_output}")
    else:
        print("Penny sikertelen lekeres: 0")

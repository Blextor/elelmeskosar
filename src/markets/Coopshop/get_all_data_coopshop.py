...
DEFAULT_API_BASE = "https://coopshop.hu/wp-json/wc/store/v1"
DEFAULT_REFERER = "https://coopshop.hu/"
DEFAULT_PER_PAGE = 100
DEFAULT_CATEGORY_PER_PAGE = 100
# A coopshop.hu egy WAF/rate limiter mögött van, ami túl gyors lekéréssorozatra
# ideiglenesen blokkolja az IP-t (a kérések válasz nélkül időtúllépnek). Ezért
# alapból kis késleltetést hagyunk az oldalak és kategóriák között.
DEFAULT_PAGE_DELAY = 0.5
DEFAULT_CATEGORY_DELAY = 1.0
...
def fetch_all_categories(session, args):
    categories = []
    page = 1
    total_pages = None
    while True:
        params = {"per_page": args.category_per_page, "page": page}
        ...
        if not page_categories or page >= total_pages:
            break
        page += 1
        if args.page_delay:
...
def build_category_rows(categories, root_slugs):
    ...
    if missing_roots:
        raise ValueError(f"Hiányzó Coopshop gyökérkategória slug(ok): {', '.join(missing_roots)}")
...
def fetch_categories(session, args):
    data = fetch_all_categories(session, args)
    categories, root_ids = build_category_rows(data, parse_root_slugs(args.root_slugs))
    if not categories:
        raise ValueError("A Coopshop kategóriafa üres.")
...
def fetch_products_for_category(session, args, category_row):
    ...
    while True:
        ...
        if total_pages is None:
            ...
            print(
                f"Coopshop kategória: {category_row['name']} ({category_id}), "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Coopshop élelmiszer termékek letöltése WooCommerce Store API-ból.")
    ...
    parser.add_argument("--category-per-page", type=int, default=DEFAULT_CATEGORY_PER_PAGE)
    ...
    parser.add_argument("--category-delay", type=float, default=DEFAULT_CATEGORY_DELAY)
...
def main():
    ...
    with requests.Session() as session:
        ...
        print(f"Coopshop kategóriafájl mentve: {category_snapshot}")
        print(f"Letöltendő Coopshop gyökérkategóriák: {len(fetch_categories_rows)}")
    ...
    print(f"{len(rows)} Coopshop termék mentve ide: {output_file}")

    failed_file = generate_filename("failed_requests")
    write_failed(failed_file, failed_rows)
    print(f"{len(failed_rows)} sikertelen Coopshop kategória mentve ide: {failed_file}")

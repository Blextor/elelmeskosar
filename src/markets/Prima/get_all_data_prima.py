...
DEFAULT_VENUE_SLUG = "online-prima-expressz-corvin"
DEFAULT_LANGUAGE = "hu"
...
def load_categories(category_file):
    ...
    for category in categories:
        category["name"] = clean_text(category.get("name", ""))
    return categories
...
def fetch_items(session, venue_slug, language, categories, page_delay, category_delay, retries, retry_delay):
    items_dict = {}
    failed_categories = []
...
def parse_args():
    parser = argparse.ArgumentParser(description="Prima termekadatok letoltese Wolt API-bol.")
    parser.add_argument("--venue-slug", default=os.environ.get("PRIMA_VENUE_SLUG", DEFAULT_VENUE_SLUG))
    ...
    parser.add_argument("--category-delay", type=float, default=0.5)

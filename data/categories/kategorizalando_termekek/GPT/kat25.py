...
def kep_letolt(termek, kepek_dir="kepek"):
    ...
    if url and not re.match(r'^https?://', url, re.IGNORECASE):
        return resolve_existing_path(url)
    extension = ".jpg"
        ...
        for row in reader:
            ...
            if image_urls_raw:
                try:
                    ...
                    img_url = ""
            if not img_url:
                local_image_paths_raw = row.get('local_image_paths') or ''
                local_image_paths = [p.strip() for p in local_image_paths_raw.split(';') if p.strip()]
                if local_image_paths:
                    img_url = local_image_paths[0]
            row['kep_url'] = img_url
...
if __name__ == '__main__':
    ...
    # kategória-JSON felderítés (az új, részletes fa előnyben; a régi fallback)
    fname_candidates = ['kategoriak_2026-06-13.json', 'kategoriak_1005.json']
    ...
    termekek = beolvas_termekek_csv('kategorizalatlan_termekek.csv')

    eredmenyek = []
    for p in ('eredmeny.json', 'eredmeny_wip_gpt_.josn', 'eredmeny_wip_claude_opus.json', 'eredmenyek.json', 'eredmeny_pekaru.json'):
        if os.path.exists(p):
